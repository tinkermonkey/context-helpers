...
def make_oura_router(collector: "OuraCollector") -> APIRouter:
    ...
    def get_readiness(
        ...
        items = collector.fetch_readiness(since=collector.resolve_push_since(since))
        return collector.apply_push_paging(items, "timestamp")
    ...
    def get_workouts(
        ...
        items = collector.fetch_workouts(since=collector.resolve_push_since(since))
        return collector.apply_push_paging(items, "start_datetime")
    ...
    def get_tags(
        ...
        items = collector.fetch_tags(since=collector.resolve_push_since(since))
        return collector.apply_push_paging(items, "timestamp")
    ...
    def get_sessions(
        ...
        items = collector.fetch_sessions(since=collector.resolve_push_since(since))
        return collector.apply_push_paging(items, "start_datetime")
